...
def calculateTargetsAt(anchor_center, txt_list, anchor_heights):
    ...
    for idx, ah in enumerate(anchor_heights):
        ...
        ratio_bbox = [0, 0, 0, 0]
        #
        ratio = float(text_bbox[0]-anchor_bbox[0]) /anchor_width
        if abs(ratio) < 1: 
            ratio_bbox[0] = ratio
        #
        # print(ratio)
        #
        ratio = float(text_bbox[2]-anchor_bbox[2]) /anchor_width
        if abs(ratio) < 1:
            ratio_bbox[2] = ratio
        #
        # print(ratio)
        #
        ratio_bbox[1] = float(text_bbox[1]-anchor_bbox[1]) /ah
        ratio_bbox[3] = float(text_bbox[3]-anchor_bbox[3]) /ah
        #
        # print(ratio_bbox)
        #
...
def getImageAndTargets(img_file, anchor_heights):
    ...
    # ///2, ///2, //3, -2
    #
    width_feat = int(ceil(ceil(ceil(img_size[0]/2.0)/2.0)/2.0))
...
def transResults(r_cls, r_ver, r_hor, anchor_heights, threshold):
    ...
    for h in range(feat_shape[0]):
        ...
        for w in range(feat_shape[1]):
            ...
            list_score.append(max(r_cls[h,w,:]))
            #
    #
    return list_bbox,list_score
    #
#
